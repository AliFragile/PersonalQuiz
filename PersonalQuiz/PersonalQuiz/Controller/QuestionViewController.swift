...
class QuestionViewController: UIViewController {
    ...
    @IBAction func singleAnswerButtonPressed(_ sender: UIButton) {
        ...
        // делаем через guard, потому что когда мы определяем индекс - это всегда возвращает опциональное значение:
    ...
    private func updateSingleStackView(using answers: [Answer]) {
        // Show single ctack view:
        singleStackView.isHidden = false
        
        // функция zip из каждой последовательности извлекает значения в свойства:
    ...
    // Подготовка 3-го сигвея, куда передаем массив результатов:
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard segue.identifier == "resultSegue" else { return }
        //умышленная возмможность упасть, если что-то пойдет не так
        let resultVC = segue.destination as! ResultsViewController
        resultVC.responses = answersChoosen     // в этот массив накапливали ответы
    }
}
